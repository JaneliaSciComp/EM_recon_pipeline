...
LOG="${LOG_DIR}/unconnected_columns.${STACK}.$(date +"%Y%m%d_%H%M%S").log"

mkdir -p ${LOG_DIR}

ARGS="org.janelia.render.client.UnconnectedColumnClient"
...
echo """
Unconnected column data will be written to:
  ${LOG}
...
grep "findUnconnectedColumns: tiles in columns" ${LOG} | sed 's/.*findUnconnectedColumns: //'
