...
Set up for slab group ${SLAB_GROUP} from project group ${PROJECT_GROUP}:
  ...
  On ${VM_IP}, run:
    ...
    ./db-restore-collections.sh --pattern '00_par.*s${FIRST_SERIAL_NUMBER}.*${SLAB_GROUP_SUFFIX}'
  ...
  After the run completes (typically 2 hours), on ${VM_IP}, run:
    ...
    ./db-dump-google-collections.sh --db render --stage 01_match --project ${PROJECT_GROUP} --slab-group ${SLAB_GROUP} --pattern icc

    # Should dump collections to:
    #  /mnt/disks/mongodb_dump_fs/dump/google/01_match/${PROJECT_GROUP}/${SLAB_GROUP}/render

    # match collection dump takes 10 minutes
    ./db-dump-google-collections.sh --db match --stage 01_match --project ${PROJECT_GROUP} --slab-group ${SLAB_GROUP} --pattern icc

    # Should dump collections to:
    #  /mnt/disks/mongodb_dump_fs/dump/google/01_match/${PROJECT_GROUP}/${SLAB_GROUP}/match
"
