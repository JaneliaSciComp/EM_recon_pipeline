...
if (( $# < 2 )); then
  echo "
USAGE $0 <number of dask client slots> <number of dask workers>     (e.g. 4 400)
...
shopt -s nullglob
DIRS=("${N5_PATH}"/*/"${RENDER_PROJECT}"/*_channel_1*/)
shopt -u nullglob # Turn off nullglob to make sure it doesn't interfere with anything later
DIR_COUNT=${#DIRS[@]}
if (( DIR_COUNT == 0 )); then
  echo "WARNING: no ${RENDER_PROJECT} project channel_1 n5 directories found in ${N5_PATH}"
elif (( DIR_COUNT == 1 )); then
  RENDERED_N5_PATH=${DIRS[0]}
else
  PS3="Choose channel_1 N5 to convert to zarr: "
...
ZARR_PATH="${RENDER_NRS_ROOT}/${VOLUME_NAME}.channel_1.zarr"  # /nrs/cellmap/data/jrc_mus-pancreas-5/jrc_mus-pancreas-5.channel_1.zarr
FINAL_ZARR_PATH="${ZARR_PATH}/recon-1/em/fibsem-${DATA_TYPE}" # /nrs/cellmap/data/jrc_mus-pancreas-5/jrc_mus-pancreas-5.channel_1.zarr/recon-1/em/fibsem-uint8
FINAL_ZARR_PATH_WITHOUT_NRS_ROOT="${FINAL_ZARR_PATH#/nrs/}"   # cellmap/data/jrc_mus-pancreas-5/jrc_mus-pancreas-5.channel_1.zarr/recon-1/em/fibsem-uint8
